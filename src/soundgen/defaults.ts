...
export const defaultOptions = {
  ...
  distortion: {
    frequency: {
      sum: (time: number, freq: number, period: number) => 1,
      factor: (time: number, freq: number, period: number) => 1,
    },
    amplitude: {
      sum: (time: number, period: number) => Math.random() * 10 - 5,
